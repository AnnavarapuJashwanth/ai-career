...
export default function DetailedPhaseCard({ phase, index, isUnlocked = true }) {
  ...
  return (
    <motion.div
      ...
      className={`relative ${isMobile ? 'bg-white' : 'bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl'} rounded-xl sm:rounded-2xl border-2 ${colors.border} overflow-hidden ${!isUnlocked ? 'opacity-60' : ''} h-full flex flex-col`}
    >
      {/* Phase Header */}
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 flex-1">
            ...
            {phase.skills?.map((skill, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 ${colors.light} ${colors.text} rounded-lg text-sm font-medium border ${colors.border} ${isMobile ? '' : 'hover:border-white/40 transition-all'} cursor-pointer`}
            ...
            <div className="space-y-3">
              {phase.resources.slice(0, 4).map((resource, idx) => (
                <motion.div
                  ...
                  className={`flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10 ${isMobile ? '' : 'hover:border-white/20 transition-all'} cursor-pointer group`}
                >
                  <div className="flex-1">
                    ...
                    <h5 className={`text-white font-medium text-sm ${isMobile ? '' : 'group-hover:text-blue-400 transition'}`}>
                      {resource.title}
                    </h5>
                  ...
                  <OpenInNew style={{ fontSize: 16 }} className={`text-gray-400 ${isMobile ? '' : 'group-hover:text-blue-400 transition'}`} />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        {isUnlocked && (
          <motion.button
            ...
            className={`w-full py-3 bg-gradient-to-r ${colors.bg} text-white font-semibold rounded-lg ${isMobile ? 'shadow' : 'shadow-lg hover:shadow-xl transition-all'}`}
          >
            ▶ Start Learning
          </motion.button>
        )}
